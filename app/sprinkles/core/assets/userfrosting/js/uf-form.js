...
(function ($, window, document, undefined) {
    ...
    Plugin.prototype._urlencodeData = function (form) {
        ...
        var serializedData = $(form).find('input, textarea, select').not(':checkbox, #input_venues_allowed, #update_input_venues_allowed, #input_zones_allowed, #venue_list, #zone_tags, #venue_tags, .ufformsubmit_skip_serialise').serialize();

        if ( $('#update_input_venues_allowed').length ) {
            var venues_data = $('#update_input_venues_allowed').select2('val');
            serializedData += '&' + encodeURIComponent('update_venues_allowed') + "=" + encodeURIComponent(venues_data);
        }

        if ( $('#input_venues_allowed').length ) {
            var venues_data = $('#input_venues_allowed').select2('val');
            serializedData += '&' + encodeURIComponent('venues_allowed') + "=" + encodeURIComponent(venues_data);
        }

        if ( $('#input_zones_allowed').length ) {
            var zones_data = $('#input_zones_allowed').select2('val');
            serializedData += '&' + encodeURIComponent('zones_allowed') + "=" + encodeURIComponent(zones_data);
        }

        if ( $('#venue_list').length ) {
            var venues_data = $('#venue_list').select2('val');
            serializedData += '&' + encodeURIComponent('venues') + "=" + encodeURIComponent(venues_data);
        }

        if ( $('#zone_tags').length ) {
            var tag_data = $('#zone_tags').select2('val');
            serializedData += '&' + encodeURIComponent('associated_tags') + "=" + encodeURIComponent(tag_data);
        }

        if ( $('#venue_tags').length ) {
            var tag_data = $('#venue_tags').select2('val');
            serializedData += '&' + encodeURIComponent('associated_tags') + "=" + encodeURIComponent(tag_data);
        }

        if ( $('#input_whitelist_vendors').length ) {
            var whitelist_device_vendor_data = $('#input_whitelist_vendors').select2('val');
            serializedData += '&' + encodeURIComponent('whitelist_vendors') + "=" + encodeURIComponent(whitelist_device_vendor_data);
        }

        $('select.ufformsubmit_skip_serialise').each(function(i) {
            if ($(this).length) {
                var array_data = $(this).select2('val');
                serializedData += '&' + encodeURIComponent(this.name) + "=" + encodeURIComponent(array_data);
            }
        });

...
namespace UserFrosting\Sprinkle\IntelliSense\Sprunje;
...
class UserSprunje extends Sprunje
{
    ...
    {
        // $query = new ExtendedUser();
        $query = $this->classMapper->createInstance('user');
        // $query = $query->joinCompany()->with('company');
     ...
     * @return self
     */
    protected function filterCompany($query, $value)
    {
        // Split value on separator for OR queries
        $values = explode($this->orSeparator, $value);
        $query->where(function ($query) use ($values) {
            foreach ($values as $value) {
                $query->orLike('company.name', $value);
            }
        });
     ...
     * @return self
     */
    protected function sortCompany($query, $direction)
    {
